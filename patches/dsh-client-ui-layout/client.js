window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* @param details - right-rail width preference in px (0 = closed); free-dragged,
		* only floored at the center column's minimum usable width.
		* @returns resolved widths; details 0 means visually closed (never unmounted), while a closed sidebar keeps its compact rail.
		*/
		function computeColumns(viewport, sidebar, details) {
			const s = sidebar === 0 ? 56 : clampWidth(sidebar, 264, 420);
			const d = details === 0 ? 0 : Math.max(0, Math.round(details));
			const centerMin = 240;
			if (s + d + centerMin <= viewport) return {
				sidebar: s,
				center: viewport - s - d,
				details: d
			};
			const capped = Math.max(0, viewport - s - centerMin);
			return {
				sidebar: s,
				center: viewport - s - capped,
				details: capped
			};
		}
		//#endregion
		...
		*/
		...
		/** Details/inspector rail column grid item; width 0 keeps both panes mounted (never unmount on close). */
		...
		*/
		...
		function AppFrame({ useStore, useSessions, actions, renderSlot }) {
			...
			const inspectorContent = renderSlot("inspector", {});
			const inspectorActive = panels.inspectorOpen && inspectorContent !== null;
			const railVisible = detailsSession !== void 0 && (panels.detailsOpen || inspectorActive);
			const cols = computeColumns(viewport, sidebarCollapsed ? 0 : panels.sidebar === 0 ? 280 : panels.sidebar, railVisible ? panels.rail : 0);
			...
			const onDetailsDrag = (0, react.useCallback)((dx) => {
				actions.setRail(detailsBase.current - dx);
			}, [actions]);
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(CenterColumn, { children: renderSlot("conversation", {}) }), (0, react_jsx_runtime.jsxs)(DetailsColumn, { children: [(0, react_jsx_runtime.jsx)("div", { style: { display: panels.detailsOpen ? void 0 : "none", flex: "1 1 auto", minWidth: 0, minHeight: 0 }, children: renderSlot("details", {}) }), (0, react_jsx_runtime.jsx)("div", { style: { display: inspectorActive ? void 0 : "none", flex: "1 1 auto", minWidth: 0, minHeight: 0 }, children: inspectorContent })] })] }),
					(0, react_jsx_runtime.jsx)("div", {
					...
					(cols.details > 0 || dragging) && (0, react_jsx_runtime.jsx)(DragHandle, {
						side: "details",
						left: cols.details > 0 ? viewport - cols.details : viewport,
		...
		*/
		function createLayoutStore() {
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					setRail: (d, px) => {
						d.rail = Math.max(0, Math.round(px));
					},
					toggleSidebar: (d) => {
					...
					openDetails: (d) => {
						d.detailsOpen = true;
						if (d.rail === 0) d.rail = 380;
					},
					closeDetails: (d) => {
						d.detailsOpen = false;
						if (!d.inspectorOpen) d.rail = 0;
					},
					openInspector: (d) => {
						d.inspectorOpen = true;
						if (d.rail === 0) d.rail = 420;
					},
					closeInspector: (d) => {
						d.inspectorOpen = false;
						if (!d.detailsOpen) d.rail = 0;
					},
					toggleInspector: (d) => {
						if (d.inspectorOpen) {
							d.inspectorOpen = false;
		...
		var LayoutController = class {
			...
			*/
			...
			openInspector() {
				this.#require().openInspector();
				this.#setInspectorOpen(true);
			}
			/** Close the inspector pane. */
			closeInspector() {
				this.#require().closeInspector();
				this.#setInspectorOpen(false);
			}
			/** Toggle the inspector pane; the mirror keeps callers in sync. */
			toggleInspector() {
				const next = !this.#inspectorOpen;
				if (next) this.#require().openInspector();
				else this.#require().closeInspector();
				this.#setInspectorOpen(next);
			}
			/** Current inspector open state (mirror; the only writers go through this face). */
			isInspectorOpen() {
				return this.#inspectorOpen;
			}
			/**
			* Subscribe to inspector open-state changes.
			* @param listener - receives the new open state.
			* @returns unsubscribe.
			*/
			onChange(listener) {
