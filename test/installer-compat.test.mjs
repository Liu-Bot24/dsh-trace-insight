...
test('release metadata and installers agree on Trace Insight 1.3.3', () => {
  ...
  assert.match(files['install.sh'], /PLUGIN_VERSION="1\.3\.3"/u)
  assert.match(files['install.sh'], /dsh-plugin-trace-insight-\$PLUGIN_VERSION\.tgz/u)
  assert.match(files['install.ps1'], /\$PluginVersion = '1\.3\.3'/u)
})
