import { assertSourceFormat, hasAnalysisCoordinates } from './source-coordinates.mjs'
...
import { TRACE_INSIGHT_SERVICE_VERSION } from './version.mjs'

export { TRACE_INSIGHT_SERVICE_VERSION }
...
export class TraceInsightService {
  ...
  async observation(sessionId, signal) {
    const observation = await readSessionObservation(this.sessionQuery, sessionId, signal)
    const version = observation.log?.session?.version
    const history = await this.store.getSession(sessionId)
    assertSourceFormat(history, version)
    if (Number.isSafeInteger(version) && version > 0 && history.sourceFormatVersion === undefined) {
      await this.store.updateSession(sessionId, current => {
        assertSourceFormat(current, version)
        if (!hasAnalysisCoordinates(current)) current.sourceFormatVersion = version
        return current
      })
    }
    return observation
